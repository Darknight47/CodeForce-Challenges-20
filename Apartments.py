...
-------------------------------------------------- Link for the challenge: https://codeforces.com/problemset/problem/1430/A ----------------------------------------

Recently a new building with a new layout was constructed in Monocarp's hometown. 
According to this new layout, the building consists of three types of apartments: three-room, five-room, and seven-room apartments. 
It's also known that each room of each apartment has exactly one window. In other words, a three-room apartment has three windows, a five-room — five windows, and a seven-room — seven windows.
...
Unfortunately, Monocarp only recently has learned to count, so he is asking you to help him to calculate the possible quantities of three-room, five-room, and seven-room apartments in the building that has n windows. 
...
for _ in range(cases):
    ...
    for a in range(n // 7 + 1):          # a = number of 7-room apartments
        for b in range(n // 5 + 1):      # b = number of 5-room apartments
            rem = n - 7*a - 5*b
            if rem >= 0 and rem % 3 == 0:
                c = rem // 3             # c = number of 3-room apartments
                print(c, b, a)
                found = True
                break
        if found:
